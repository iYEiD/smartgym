...
CREATE INDEX IF NOT EXISTS idx_check_in_timestamps ON check_in_logs(check_in_time, checkout_time);
...
RETURNS TRIGGER AS $$
BEGIN
    NEW.duration_minutes = EXTRACT(EPOCH FROM (NEW.checkout_time - NEW.check_in_time))/60;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
FOR EACH ROW
WHEN (OLD.checkout_time IS NULL AND NEW.checkout_time IS NOT NULL)
